...
function main() {
  ...
  initSlide($('#banner'), 3, 4, 'swing');
  initSlide($('#banner2'), 3, 5, 'swing');

  function initSlide($banner, showSlides, timeSlide, ease) {
    var

      heightContent = $banner.outerHeight(),
      activeSlides = showSlides,
      $slides = $banner.find('.slide'),
      heightSlides = heightContent / activeSlides;

    colocarSlides($slides, heightSlides);
    animateSlides($banner, heightSlides, timeSlide, ease);
  }

  function animateSlides($contentSlides, heightSlides, timeSlide, ease) {
    var
      $mySlides = $contentSlides.find('.slide'),
      $lastSlide = $($mySlides[$mySlides.length - 1]),
      $lastSlideClone = $lastSlide.clone();
    //colocarSlides($mySlides, heightSlides);
    //clonamos y agregamos arriba
    $contentSlides.prepend($lastSlideClone);
    $lastSlideClone.css({
      top: -heightSlides
    });
    //volvemos a capturar la lista con todos los items
    $mySlides = $contentSlides.find('.slide');
    $mySlides.each(function(i, ele) {
      var
        $ele = $(ele),
        topElement = $ele.position().top;
      $ele.animate({
        top: topElement + heightSlides
      }, timeSlide * 1000, ease, function() {
        if (i == $mySlides.length - 1) {
          animateSlides($contentSlides, heightSlides, timeSlide);
        }
      });
    });
  ...
  function colocarSlides(slides, heightSlide) {
